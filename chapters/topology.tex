...
In a similar vein, when we just have a manifold $M^n$, a \defn{collar neighborhood} of its boundary is a neighborhood $\widetilde{\partial M}\supset \partial M$ which is the diffeomorphic image of a trivial ``ray-bundle'' $\partial M\times [0,\infty)$. As for tubular neighborhoods, we require that the diffeomorphism is the identity on $\partial M\times\{0\}$.
...
Let us suppose we have manifolds $M_1^n$ and $M_2^n$, with embeddings $\iota_1 : N\to M_1$ and $\iota_2 : N\to M_2$ of a manifold $N^k$. Our goal is to construct a joined manifold $M$ which we might denote $M_1\cup_N M_2$.
We start by picking tubular neighborhoods $\widetilde{\iota_i N}$ for $\iota_i(N)\subset M_i$ and let $\tau_i: N\times \R^{n-k} \to \widetilde{\iota_i N}$ be the diffeomorphism.
...
The resulting join is an operation known as a \defn{connected sum} of two manifolds $M_1$ and $M_2$. If the manifolds are connected, this operation is (up to orientation-preserving orientation) independent of the choice of embedding. This latter statement is non-trivial, and follows from a technical result by Richard Palais.

\begin{theorem}[Disk Theorem]
	If $M^n$ is an oriented manifold with orientation preserving embeddings $\iota_1, \iota_2 : \Int(D^n) \to M$ of an open disk, then there is a diffeomorphism $f : M \to M$ such that $\iota_1 = f\circ \iota_2$.
\end{theorem}
\begin{proof}
