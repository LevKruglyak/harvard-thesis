...
\begin{proof}
	...
	For the Pontryagin numbers, we note that just as with the Euler number, the map $p_1 : \pi_3(\SO_4) \to \H^4(X)$ is a group homomorphism and linear in $(i,j)$. Under the reflection $(i,j)\mapsto (-i,-j)$, the map is inverted $(i+j)
	\todo{finish}
\end{proof}
