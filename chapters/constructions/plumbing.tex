...
Thus far, we have only seen exotic spheres in $\bP^{4k}$. This is not the only dimensions in which exotic spheres exist, there is an additional tricky class of exotic spheres living in $\bP^{4k+2}$. The basic plumbing construction used here is:
...
As suggested by its name, this is a homotopy sphere: 

\begin{theorem}
	There is a group homomorphism
	\[
		\lkxfunc{}{\Z/2}{\bP^{4k+2}}{n}{n\cdot \partial P^{4k+2}(A_2).}
	\]
	In particular, note that $\partial P^{4k+2}(A_2)\+ \partial P^{4k+2}(A_2) \cong S^{4k+1}$.
\end{theorem}
\begin{proof}
	See Theorem V.2.11 in \cite{browder1972surgery}.
\end{proof}
