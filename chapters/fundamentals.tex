...
By the mid 1940s, these ideas were expanded by Lev Pontryagin and Shing-Shen Chern to better capture the geometric data of oriented real and complex vector bundles respectively. The theory of characteristic classes is incredibly rich, with applications ranging from algebraic topology, to the differential topology of exotic spheres, to the study of elliptic differential operators in differential geometry. As such, there are many equivalent ways to construct and characterize the various notions of characteristic class which arise in mathematics. 

\subsection{Axiomatic Definitions of Characteristic Classes}

At an abstract level a \defn{characteristic class} $c$, valued in a cohmology theory $h$, is a natural transformation of contravariant functors
\begin{equation}
	\lkxfunc{c}{\Vect_G}{h^k,}
\end{equation}
given a structure group $G$. Here, $h^k$ sends a space\footnote{The choice of category for topological spaces here is flexible -- we might consider the category of paracompact Hausdorff spaces for instance.} to its $k$-th cohomology group in the theory $h$, and $\Vect_G$ sends a space to the set of isomorphism classes of vector bundles over the space with structure group $G$.

In other words, given a manifold $M$, a characteristic class assigns a vector bundle $\xi$ over $M$ to a cohomology class $c(\xi)\in h^k(M)$.
This assignment should be natural, i.e. given bundles $\xi_1$ and $\xi_2$ over manifolds $M_1$ and $M_2$, whenever a map $f : M_1 \to M_2$ is covered by a bundle map $\xi_1 \to \xi_2$, we have $f^* c(\xi_2) = c(\xi_1)$.

\begin{remark}
	Since every smooth manifold
\end{remark}

\subsection{Stiefel-Whitney Classes}
The simplest characteristic classes are the Stiefel-Whitney classes. We will define them axiomatically, however a construction will still be required.
\begin{definition}
	The \defn{Stiefel-Whitney classes} $w_i$ are the unique sequence of characteristic classes for unoriented vector bundles $(G=\O)$ satisfying the following axioms:
	\begin{enumerate}
		\item For a bundle $\xi$ over an $n$-dimensional manifold, $w_0(\xi)=1$ and $w_i(M)=0$ for $i>n$.
		\item  For bundles $\xi_1$ and $\xi_2$ over a common base, we have the \defn{Whitney product formula}
		      \[
			      w_i(\xi_1\oplus \xi_2) = \sum_{p+q=i} w_p(\xi_1)\smile w_q(\xi_2).
		      \]
		\item The first Stiefel-Whitney
	\end{enumerate}
\end{definition}



\subsection{A Universal Perspective}
