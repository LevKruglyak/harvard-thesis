...
\begin{remark*}
	Many sources define characteristic classes first as a set of homogeneous classes $c_i\in h^i(\mathcal{E})$ and then refer to the \defn{total characteristic class} $c(\mathcal{E})=\sum_i c_i(\mathcal{E})$. Hwoever, throughout this section 
\end{remark*}
...
If the ring is graded-commutative we can expand \cref{eq:formal-inversion} to get: 
\[
	\begin{aligned}
		y_0 & = x_0^{-1},                        \\
		y_1 & = -x_1x_0^{-2}                     \\
...
\begin{definition}\label{def:characteristic-number}
	Given any characteristic class $c$ and closed $n$-dimensional manifold $M$, the \defn{characteristic number} of $M$ is $c[M] = c_n(\TT M)[M]$ where $|c_n|=n$.
\end{definition}
...
These generators are one construction of the \defn{Chern classes}, cohomology classes for complex vector bundles with coefficients $\Z$ in singular homology. The properties of these characteristic classes are directly tied to the group theoretic properties of $\U_m$.
...
Note that all choices of maximal tori are conjugate, so the exact choice of maximal torus does not affect the structure of the Weyl group.
...
\begin{definition}
	A \defn{multiplicative sequence} $K=\{K_n\}$ is a set of polynomials in which $K_n(\sigma_1,\ldots, \sigma_n)$ is homogeneous of degree $n$ and $K$ induces an endomorphism of $R\fps{\sigma_1,\sigma_2,\ldots}^\times$.
\end{definition}
...
Now given a real vector bundle $\mathcal{E}$, complexify it to get a complex vector bundle $\mathcal{E}_\C = \mathcal{E}\otimes \C$. 
...
\begin{definition}
	The \defn{Pontryagin class} $p$ is the image of $p'$ in $\H^\bullet(\BSO_{2m}; \Z[\frac{1}{2}])$. Let $p_n$ denote $p'_{2n}$, so each the Pontryagin class $p_n$ has degree $4n$. Note that we have the relation
	\[
...
\]
on the generators of $\Omega_\bullet^\SO\otimes \Q$. If we can find some multiplicative characteristic class which evaluates to $1$ on $\CP^{2k}$, we would be able to express the signature in terms of characteristic classes. A ring homomorphism from a cobordism ring is generally known as a \defn{genus}.
...
Luckily for us, the coefficient of this leading term admits a description in terms of \defn{Bernoulli numbers $B_{2k}$}, a sequence of rational numbers that appear ubiquitously throughout topology, homotopy theory, number theory, and many other disciplines. For an overview o some of these connections, see \cite{mazurbernoulli} or Appendix B of \cite{milnorstasheff1974}.
